...
export function ScaffoldView({ scaffold, writingFrame, question, marks, initialViewMode = 'scaffold' }: ScaffoldViewProps) {
  ...
  const exportPrefix = viewMode === 'scaffold' ? 'IGCSE_Scaffold' : 'IGCSE_Writing_Frame';
    ...
    try {
      ...
      printContainer.innerHTML = `
        <div style="padding: 40px; font-family: Arial, sans-serif; color: #334155; background: #ffffff;">
          <div style="margin-bottom: 30px; padding: 24px; background-color: #f8fafc; border-left: 6px solid #2563eb; border-radius: 4px;">
      ...
      const sanitize = (node: Element) => {
        if (node instanceof HTMLElement) {
          ...
          if (tag === 'h1') {
            ...
            node.style.borderBottom = '2px solid #2563eb';
            ...
            node.style.borderLeft = '4px solid #2563eb';
      ...
      while (yPositionPx < canvas.height) {
        ...
        if (sliceCtx) {
          ...
          pdf.text(`IGCSE Student Scaffold • Page ${pageCount + 1}`, pdfWidth / 2, pdfHeight - 15, { align: 'center' });
        }
  ...
  const exportAsDocx = async () => {
    ...
    try {
      const header = `
        ...
        <head>
          <meta charset='utf-8'>
          <title>IGCSE Essay Scaffold</title>
          <style>
            ...
            h1 { font-size: 24pt; border-bottom: 2px solid #2563eb; padding-bottom: 10px; }
            h2 { font-size: 18pt; color: #2563eb; }
            h3 { font-size: 14pt; border-left: 4px solid #2563eb; padding-left: 15px; }
            p { margin: 1em 0; }
  ...
  return (
    ...
    >
      <div className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-200 dark:border-slate-800 px-4 py-2 flex justify-between items-center shrink-0">
            ...
            <>
              ...
              <div className="flex bg-slate-100 dark:bg-slate-900 rounded-lg p-0.5 border border-slate-200 dark:border-slate-800">
                <button 
                  ...
                  className={`px-3 py-1 text-[9px] font-black uppercase tracking-tighter rounded-md transition-all ${viewMode === 'scaffold' ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600'}`}
                >
                  Scaffold
                </button>
                <button 
                  onClick={() => setViewMode('frame')}
                  className={`px-3 py-1 text-[9px] font-black uppercase tracking-tighter rounded-md transition-all ${viewMode === 'frame' ? 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-400 dark:text-slate-500 hover:text-slate-600'}`}
                >
                  Writing Frame
                </button>
              </div>
        ...
        <div className="relative">
          <button 
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-[10px] font-bold text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded uppercase tracking-wider transition-all disabled:opacity-50"
          >
          ...
          <AnimatePresence>
            {isMenuOpen && (
              <>
                ...
                >
                  <button onClick={exportAsPdf} className="w-full px-4 py-2 text-left text-[10px] font-bold text-slate-600 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400 flex items-center gap-2 uppercase tracking-wide transition-colors">
                    <FilePdf className="w-3.5 h-3.5" />
                    Download PDF
                  </button>
                  <button onClick={exportAsDocx} className="w-full px-4 py-2 text-left text-[10px] font-bold text-slate-600 dark:text-slate-400 hover:bg-blue-50 dark:hover:bg-blue-900/20 hover:text-blue-600 dark:hover:text-blue-400 flex items-center gap-2 uppercase tracking-wide transition-colors">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 p-6 md:p-8 custom-scrollbar bg-white dark:bg-slate-900">
        <div className="mb-8 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-800 border-l-4 border-l-blue-600">
          <p className="text-sm md:text-base font-semibold text-slate-800 dark:text-slate-100 leading-relaxed italic">
            {question} <span className="text-blue-600 dark:text-blue-400">[{marks}]</span>
          </p>
        </div>
        
        <div className="prose prose-slate dark:prose-invert max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-p:text-slate-600 dark:prose-p:text-slate-400 prose-li:text-slate-600 dark:prose-li:text-slate-400 prose-strong:text-slate-900 dark:prose-strong:text-slate-200 prose-headings:text-blue-900 dark:prose-headings:text-blue-400 prose-hr:border-slate-100 dark:prose-hr:border-slate-800" ref={contentRef}>
          <Markdown>{sanitizedContent}</Markdown>
        </div>
      </div>

      <div className="bg-blue-50 dark:bg-blue-900/10 px-4 py-2.5 border-t border-blue-100 dark:border-blue-900/20 flex items-center gap-2 shrink-0">
        <CheckCircle className="w-3.5 h-3.5 text-blue-600 dark:text-blue-500" />
        <p className="text-[10px] font-medium text-blue-800 dark:text-blue-400 uppercase tracking-wide">
