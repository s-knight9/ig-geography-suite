    ...
    <div className="flex flex-col h-screen w-full bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 overflow-hidden transition-colors duration-300">
      ...
      <header className="h-20 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 shrink-0 relative z-20 transition-colors">
        <div className="flex items-center gap-5">
          <div className="w-12 h-12 bg-[#2563eb] rounded-xl flex items-center justify-center text-white shadow-lg shadow-blue-200 dark:shadow-blue-900/20 cursor-default select-none">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 shadow-sm flex flex-col gap-6 flex-1 transition-colors">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-4">
              <h2 className="text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-2">
                <div className="w-1 h-3 bg-[#2563eb] rounded-full" /> ASSESSMENT DATA ENTRY
              </h2>
              <span className="text-[10px] font-black px-2 py-0.5 bg-blue-50 dark:bg-blue-900/10 text-[#2563eb] rounded uppercase tracking-tighter">{activeTeacherCode || "SKN"}</span>
            </div>
            ...
            <div className="space-y-6 flex-1">
              ...
              <div className="space-y-3 pt-4 border-t border-slate-100 dark:border-slate-800">
                ...
                <select 
                  ...
                  className={`w-full bg-slate-50 dark:bg-slate-850 border border-slate-200 dark:border-slate-700 rounded-lg px-4 py-2.5 text-xs font-black focus:ring-2 focus:ring-blue-500 outline-none transition-colors ${framework === 'PEECAL' ? 'text-blue-600 dark:text-blue-400' : 'text-slate-600 dark:text-slate-400'}`}
                >
                  <option>PEE</option>
                  <option>PEEL</option>
              ...
              <div className="flex items-center justify-between py-4 border-y border-slate-100 dark:border-slate-800">
                ...
                <button 
                  ...
                  className={`w-11 h-6 rounded-full relative transition-all shadow-inner ${useWordBank ? 'bg-[#2563eb]' : 'bg-slate-200 dark:bg-slate-700'}`}
                >
        ...
        <div className="flex-1 flex flex-col gap-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-[32%] shrink-0">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm flex flex-col group transition-colors">
              <label className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-3 flex justify-between items-center px-1">
                ...
                <span className="w-1.5 h-1.5 bg-blue-400 rounded-full group-focus-within:animate-ping" />
              </label>
              <textarea 
                value={question}
                onChange={(e) => setQuestion(e.target.value)}
                className="flex-1 w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-5 text-sm font-medium resize-none focus:ring-2 focus:ring-blue-500 focus:bg-white dark:focus:bg-slate-800 outline-none leading-relaxed transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600 dark:text-slate-200"
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 shadow-sm flex flex-col group transition-colors">
              ...
              <textarea 
                ...
                className="flex-1 w-full bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-5 text-xs font-semibold resize-none focus:ring-2 focus:ring-blue-500 focus:bg-white dark:focus:bg-slate-800 outline-none leading-relaxed transition-all placeholder:text-slate-300 dark:placeholder:text-slate-600 dark:text-slate-200"
            ...
            <AnimatePresence mode="wait">
              {loading ? (
                <motion.div 
                  ...
                  animate={{ opacity: 1 }} 
                  exit={{ opacity: 0 }}
                  className="absolute inset-0 z-10 bg-white/60 dark:bg-slate-950/60 backdrop-blur-[2px] rounded-xl flex flex-col items-center justify-center p-12 border border-blue-100 dark:border-blue-900/30"
                >
                  <Loader2 className="w-10 h-10 text-blue-500 animate-spin mb-4" />
                  <h3 className="text-sm font-bold text-blue-900 dark:text-blue-400 uppercase tracking-widest animate-pulse">Analyzing Syllabus context...</h3>
                </motion.div>
              ) : error ? (
                <motion.div 
                   ...
                   <div className="flex items-center gap-6 mb-6">
                     ...
                     <div className="flex flex-col items-center">
                        <div 
                          ...
                          className={`w-28 h-28 flex flex-col items-center justify-center border-2 border-dashed rounded-2xl bg-slate-50/50 dark:bg-slate-800/20 text-center p-4 cursor-pointer hover:bg-blue-50/50 dark:hover:bg-blue-900/10 transition-all group ${file ? 'border-[#2563eb] bg-blue-50/50 dark:bg-blue-900/20' : 'border-slate-100 dark:border-slate-800 hover:border-blue-200 dark:hover:border-blue-700'}`}
                        >
                          {file ? (
                            <div className="text-blue-600 dark:text-blue-400 space-y-1">
                               <CheckCircle className="w-5 h-5 mx-auto" />
                               <p className="text-[7px] font-black uppercase tracking-tight text-[#2563eb] line-clamp-2 px-1">{file.name}</p>
                           ...
                           <>
                             <FileUp className="w-5 h-5 text-slate-300 dark:text-slate-600 mb-2 group-hover:text-blue-400 transition-colors" />
          ...
          <div className="flex flex-col md:flex-row items-center gap-3 h-auto md:h-14 mt-6 w-full shrink-0">
            <button 
              ...
              disabled={loading}
              className="flex-1 w-full h-full min-h-[48px] px-6 bg-[#2563eb] hover:bg-[#1d4ed8] disabled:bg-blue-300 dark:disabled:bg-blue-900/50 text-white font-black text-xs uppercase tracking-[0.15em] rounded-xl shadow-lg shadow-blue-100 dark:shadow-blue-900/10 transition-all active:scale-[0.99] flex items-center justify-center gap-2 group"
            >
